...
<div class="intro-y flex items-center mt-8">
    <h2 class="text-lg font-medium mr-auto">Editar Region</h2>
</div>
@if(session('update'))
...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 lg:col-span-6">
        <form method="POST" action="/admin/regiones-update">
            @csrf
            <!-- BEGIN: Form Layout -->
            <div class="intro-y box p-5">
                ...
                <div class="mt-3">
                    ...
                    @if(count($data['paises'])>0)
                        ...
                        <option @if($data['data']->country == $value->id) selected @endif value="{{$value->id}}">{{$value->name}}</option>
                ...
                <div class="mt-3">
                    ...
                    <input id="crud-form-1" name="name" type="text" value="{{$data['data']->name}}" class="form-control w-full" placeholder="Nombre Region"
                        required>
                </div>
                <div class="mt-3">
                    <label for="crud-form-1" class="form-label">Color Hexadecimal (#FFFFFF)</label>
                    <input id="crud-form-1" name="color" value="{{$data['data']->color}}" type="text" class="form-control w-full" placeholder="#FFFFFF"
                        required>
                </div>
