...
@section('subhead')
<title>Clientes - Usuario - SIMEP</title>
@endsection
        ...
        <div class="grid grid-cols-12 gap-6">
            ...
            <div class="col-span-12 md:col-span-12 mt-8">
                <div class="intro-y items-center ">
                    <h2 class="text-xl font-bold truncate mr-5 w-full title-section">Clientes</h2>
                ...
                <div class=" mt-8">
                    <div class=" px-2">
                        <div class="h-full  rounded-md">
                            ...
                            <p>Informes y reportes de estudios y analisis para clientes (Puntos de ventas) de Empresas Polar.</p>
                        </div>
                    </div>
            ...
            <div class="col-span-12 md:col-span-12">
                ...
                <div class="intro-y col-span-12 box p-5 mt-8 overflow-auto lg:overflow-visible">

                    <div class="grid grid-cols-12 gap-2 mb-4">
                    <select id="filter-canal" class="form-select form-select-lg sm:mt-2 sm:mr-2 col-span-6 sm:col-span-3 xl:col-span-3"
                            aria-label=".form-select-lg example">
                            <option value="">Canal</option>
                            @if(count($data['canales']) > 0)
                            @foreach($data['canales'] as $key => $value)

                            <option @if($data['current_canal'] == $value->canal) selected @endif  value="{{$value->canal}}">{{$value->canal}}</option>
                            @endforeach
                            @endif
                        </select>
...
$(document).on("change", "#filter-year, #filter-canal", function () {

jQuery.noConflict();
console.log("Cambio Año")
var ano = jQuery("#filter-year").val();
var categoria = jQuery("#filter-canal").val();
...
let loader = document.querySelector('.loader')
   ...
   jQuery.ajax({
               type: 'POST',
               url: "{{ url('usuario/filtro-clientes') }}",
               data: {
                   "_token": "{{ csrf_token() }}",
                   ano: ano,
                   categoria: categoria
