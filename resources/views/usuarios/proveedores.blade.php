...
@section('subhead')
<title>Proveedores - Usuario - SIMEP</title>
@endsection
        ...
        <div class="grid grid-cols-12 gap-6">
            ...
            <div class="col-span-12 md:col-span-12 mt-8">
                <div class="intro-y items-center ">
                    <h2 class="text-xl font-bold truncate mr-5 w-full title-section">Proveedores</h2>

                    <div class=" mt-8">
                    <div class="px-2">
                        <div class="h-full  rounded-md">
                          
                            <p>Sección que contiene información suministrada por los distintos proveedores de data, como, por ejemplo: </p>
                            <p>Nielsen: Auditorías de mercado para categorías de interés (Retail, Scantrack y Backdata)</p>
                            <p>Atenas Consultores: Participación de mercado por áreas geográfica para categorías de interés</p>
                            <p>Euromonitor: tamaño de mercado para categorías de Alimentos empacados en USA, Canadá y LATAM</p>
                        </div>
                    </div>
            ...
            <div class="col-span-12 md:col-span-12">
                ...
                <div class="intro-y col-span-12 overflow-auto box p-5 lg:overflow-visible">
                    ...
                    <select id="filter-proveedor" class="form-select form-select-lg sm:mt-2 sm:mr-2 col-span-6 sm:col-span-3 xl:col-span-3"
                            aria-label=".form-select-lg example">
                            <option value="">PROVEEDOR</option>
                            @if(count($data['proveedores']) > 0)
                            @foreach($data['proveedores'] as $key => $value)

                            <option @if($data['current_proveedor'] == $value->proveedor) selected @endif value="{{$value->proveedor}}">{{$value->proveedor}}</option>
                            @endforeach
                            @endif
                        </select>
                        <select id="filter-year" class="form-select form-select-lg sm:mt-2 sm:mr-2 col-span-6 sm:col-span-3 xl:col-span-3"
                            aria-label=".form-select-lg example">
                    ...
                    <div class="overflow-x-auto">
                        <table class="table table-report mt-4 col-span-6" id="table-proveedores">
                            <thead>
                                <tr>
                            ...
                            <tbody id="table-proveedores-content">
...
if(isChrome){
    ...
    jQuery.noConflict();
    jQuery('#table-proveedores').DataTable({
    "aaSorting": [[ 0, "asc" ]],
    "pageLength": 25
    });
...
$(document).on("change", "#filter-year, #filter-proveedor", function () {

jQuery.noConflict();
console.log("Cambio Año")
var ano = jQuery("#filter-year").val();
var categoria = jQuery("#filter-proveedor").val();
...
let loader = document.querySelector('.loader')
   ...
   jQuery.ajax({
               type: 'POST',
               url: "{{ url('usuario/filtro-proveedores') }}",
               data: {
                   "_token": "{{ csrf_token() }}",
                   ano: ano,
                   proveedor: categoria
               ...
               success: function (data) {
                   ...
                   jQuery('#table-proveedores').DataTable().destroy();
                    jQuery('#table-proveedores').find('tbody').html("");
                    jQuery('#table-proveedores').find('tbody').append(element);
                    jQuery('#table-proveedores').DataTable({
                    "aaSorting": [[ 0, "asc" ]],
                    "pageLength": 25
                    }).draw();
